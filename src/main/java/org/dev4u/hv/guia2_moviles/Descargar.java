...
public class Descargar extends AsyncTask<String, Integer, Estado> {
    ...
    private boolean Cambiar_nombre;



    public Descargar(Context context, TextView estado, ProgressBar progressBar,
                     boolean cambiar_nombre, String nuevo_nombre, Button descargar) {
        this.context      = context;
        this.lblEstado    = estado;
        this.btnDescargar = descargar;
        this.barra = progressBar;
        this.nuevo_nombre = nuevo_nombre;
        this.Cambiar_nombre = cambiar_nombre;
    ...
    protected Estado doInBackground(String... sUrl) {
        ...
        try {
           ...
           //*************************************************************************************
            if (Cambiar_nombre ==true){
                //se obtiene extension de la imagen que se descarga
                String extension = nombre.substring(nombre.length()-4, nombre.length());

                nombre = nuevo_nombre; //se le pasa a nombre el nuevo nombre si es verdadero cambiar nombre
                ruta = Environment
                        .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
                        .getAbsolutePath()+ "/"+ nombre+extension;
            }else {
                ruta = Environment
                        .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
